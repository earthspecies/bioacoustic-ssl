...
from bioacoustic_ssl.data import Compose, compute_sample_weights
...
def main(fabric: Fabric, cfg: DictConfig) -> None:
    ...
    train_cfg = instantiate(cfg.data.datasets["train"])
    val_cfg = instantiate(cfg.data.datasets["validation"])
    test_cfg = instantiate(cfg.data.datasets["test"])

    # Share ONE label map across splits. Otherwise each split builds its own map
    # from its unique labels, so a class present in train but not in test yields
    # train indices >= (test class count) -> CUDA device-side assert in the loss
    # (hit on cbi: train 264 classes, test 263). The sorted union also keeps
    # train/val/test indices aligned so the reported accuracy is meaningful.
    label_map = _shared_label_map(train_cfg, val_cfg, test_cfg)
    if label_map is not None:
        for ds_cfg in (train_cfg, val_cfg, test_cfg):
            _set_label_map(ds_cfg, label_map)

    train_dataset, train_meta = dataset_from_config(train_cfg)
    val_dataset, _ = dataset_from_config(val_cfg)
    test_dataset, test_meta = dataset_from_config(test_cfg)
    ...
    # Validation split uses the same (augmentation-free) "test"-stage transforms.
    ...
    val_metrics = build_stages_list(list(cfg.module.metrics.values()), "test")
...
def train(fabric: Fabric,
    ...
    # Model selection is driven by the validation split; the test metric recorded
    # at the best-val step is written to run.summary (avoids selecting on test).
    ...
    while global_step < max_steps:
        for batch in train_loader:
            ...
            if global_step % validate_every == 0:
                _evaluate_and_track(fabric, model, criterion, val_loader, val_metrics,
                                    test_loader, test_metrics, run, global_step, best)
    ...
    # Ensure the final step is evaluated even if it is not a validate_every multiple.
    if global_step % validate_every != 0:
        _evaluate_and_track(fabric, model, criterion, val_loader, val_metrics,
                            test_loader, test_metrics, run, global_step, best)

    if fabric.is_global_zero and best["step"] >= 0:
        log.info(f"Best val {SELECT_METRIC}={best['val']:.4f} at step {best['step']}")
...
# Metric used for validation-based model selection (higher is better). Falls
# back to the first configured metric when this one is not present.
SELECT_METRIC = "MulticlassAccuracy"


def _evaluate_and_track(fabric: Fabric,
                        model: ViTClassifier,
                        criterion,
                        val_loader: DataLoader,
                        val_metrics: list,
                        ...
                        global_step: int,
                        best: dict) -> None:
    """Evaluate val + test at ``global_step`` and record the test metric at the
    best validation step into ``run.summary`` (in-place update of ``best``)."""
    val_results = validate(fabric, model, val_loader, criterion, val_metrics, run, global_step, prefix="val")
    test_results = validate(fabric, model, test_loader, criterion, test_metrics, run, global_step, prefix="test")
    model.train()

    if not fabric.is_global_zero or not val_results:
        return

    select_val = val_results.get(SELECT_METRIC, next(iter(val_results.values())))
    if select_val > best["val"]:
        best["val"] = select_val
        best["step"] = global_step
        run.summary[f"val/best_{SELECT_METRIC}"] = select_val
        run.summary["val/best_step"] = global_step
        for name, value in test_results.items():
            run.summary[f"test/best_{name}"] = value
...
def validate(fabric: Fabric,
    ...
    results = {}
    if fabric.is_global_zero:
        results = {metric.__class__.__name__: float(metric.compute()) for metric in metrics}
        run.log({f"{prefix}/loss": loss.item(),
...
def _label_from_feature_cfg(ds_cfg):
    """Return the ``label_from_feature`` transformation config for a split, or None."""
    for t in (ds_cfg.transformations or []):
        if getattr(t, "type", None) == "label_from_feature":
            return t
    return None


def _shared_label_map(*ds_cfgs) -> dict | None:
    """Deterministic ``{label: idx}`` map over the union of labels across splits.

    Returns None when the splits don't use a ``label_from_feature`` transform
    (e.g. the multilabel birdset configs), leaving the original behaviour intact.
    """
    feats = [_label_from_feature_cfg(c) for c in ds_cfgs]
    if any(f is None for f in feats):
        return None
    # A config-provided label_map (e.g. the anchored map in cbi.yaml) is
    # authoritative: leave the splits untouched and skip the extra builds.
    if all(f.label_map is not None for f in feats):
        return None
    labels: set = set()
    for ds_cfg in ds_cfgs:
        _, meta = dataset_from_config(ds_cfg)
        labels.update(meta["label_from_feature"]["label_map"].keys())
    return {lbl: idx for idx, lbl in enumerate(sorted(labels))}


def _set_label_map(ds_cfg, label_map: dict) -> None:
    """Force a split's ``label_from_feature`` transform to use ``label_map``."""
    f = _label_from_feature_cfg(ds_cfg)
    if f is not None:
