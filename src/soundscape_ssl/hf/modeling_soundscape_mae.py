"""The released Soundscape-MAE encoder and its Xeno-Canto classification head.
...
re-implementation that happens to agree. ``tests/unittests/test_hf_soundscape_mae.py``
...
class SoundscapeMAEPatchEmbed(nn.Module):
    """Non-overlapping patch projection of a mel spectrogram to token embeddings."""

    def __init__(self, config: SoundscapeMAEConfig) -> None:
...
class SoundscapeMAEAttention(nn.Module):
...
class SoundscapeMAEMLP(nn.Module):
...
class SoundscapeMAELayer(nn.Module):
    ...
    def __init__(self, config: SoundscapeMAEConfig) -> None:
        ...
        self.attn = SoundscapeMAEAttention(config)
        self.norm2 = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.mlp = SoundscapeMAEMLP(config)
...
class SoundscapeMAEPreTrainedModel(PreTrainedModel):
    """Shared HuggingFace plumbing: config class, weight init, input name."""

    config_class = SoundscapeMAEConfig
    config: SoundscapeMAEConfig
    base_model_prefix = "encoder"
    main_input_name = "input_values"
    _no_split_modules = ["SoundscapeMAELayer"]
    ...
    def _init_weights(self, module: nn.Module) -> None:
        ...
        if isinstance(module, nn.Linear | nn.Conv2d):
            ...
            nn.init.zeros_(module.bias)
        elif isinstance(module, SoundscapeMAEModel):
            nn.init.trunc_normal_(module.cls_token, std=std)


class SoundscapeMAEModel(SoundscapeMAEPreTrainedModel):
    ...
    :class:`SoundscapeMAEFeatureExtractor` produces from audio. Feeding anything
    ...
    def __init__(self, config: SoundscapeMAEConfig) -> None:
        ...
        self.patch_embed = SoundscapeMAEPatchEmbed(config)
        ...
        self.blocks = nn.ModuleList(
            [SoundscapeMAELayer(config) for _ in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        Args:
            input_values: ``(batch, 1, num_mel_bins, num_frames)`` spectrogram,
                as returned by :class:`SoundscapeMAEFeatureExtractor`.
...
class SoundscapeMAEPrototypicalHead(nn.Module):
...
class SoundscapeMAEForAudioClassification(SoundscapeMAEPreTrainedModel):
    ...
    def __init__(self, config: SoundscapeMAEConfig) -> None:
        ...
        super().__init__(config)
        self.encoder = SoundscapeMAEModel(config)
        ...
        self.head = SoundscapeMAEPrototypicalHead(config)
        self.post_init()
