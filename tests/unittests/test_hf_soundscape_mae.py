...
transformers-version conflicts::
    ...
    uv run --with "transformers==4.57.1" pytest tests/unittests/test_hf_soundscape_mae.py
"""
...
@pytest.fixture
def config() -> SoundscapeMAEConfig:
...
@pytest.fixture
def spectrogram(config: SoundscapeMAEConfig) -> torch.Tensor:
...
def test_encoder_reproduces_repo_encoder(config: SoundscapeMAEConfig, spectrogram: torch.Tensor) -> None:
    """The published encoder is bit-exact against ``ViTEncoder``."""
    torch.manual_seed(1)
    reference = ViTEncoder(**GEOMETRY).eval()
    published = SoundscapeMAEModel(config).eval()
...
def test_encoder_hidden_states_are_the_pre_norm_block_outputs(
    config: SoundscapeMAEConfig, spectrogram: torch.Tensor
) -> None:
...
def test_classifier_reproduces_repo_classifier(config: SoundscapeMAEConfig, spectrogram: torch.Tensor) -> None:
    ...
    published = SoundscapeMAEForAudioClassification(config).eval()
...
def test_auto_classes_round_trip(
    tmp_path, config: SoundscapeMAEConfig, spectrogram: torch.Tensor
) -> None:
    ...
    torch.manual_seed(3)
    classifier = SoundscapeMAEForAudioClassification(config).eval()
